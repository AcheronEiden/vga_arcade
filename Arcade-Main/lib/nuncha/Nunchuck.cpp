...
static void mydelay() {
    // uint8_t n;
    // // int dadda=0;
    // for (n = 1; n < 254; ++n) { //TH: Count to 100 gives 700us delay?
    //     PORTB ^= 1; //TH: Toggle portB bit 0 // Just to DO something

    //     // dadda+=1; //TH: Just to DO something
    // }
}
...
uint8_t NunchuckInterface::begin()
{
  ...
  Wire.setWireTimeout(2800, false); //TH: 'false' gives data on bus from Nunchuk, 'true' does not.
  ...
  Wire.write ( (uint8_t) 0x55 ); //TH: org was 0xAA
  wireErr = Wire.endTransmission();
  if ( wireErr ) return ( wireErr );
  Wire.beginTransmission ( NCDevID ); //TH: Extra block here!
